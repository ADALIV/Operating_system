...
frame* RAM;
bool free_page_list[TOTAL_PAGE_SIZE] = {0};
...
void initialize_free_page_list() {
    ...
    for (int i = 0; i < TOTAL_PAGE_SIZE; i++) {
        free_page_list[i] = true; // 모든 프레임을 사용 가능으로 초기화
    }
}
...
int translate_VA_to_PA(int** TTBR, int va) {
    ...
    // 물리 주소 생성
    // char pa_value = RAM[L2[l2_index]].block[offset];
...
void handle_page_fault(int** TTBR, int va, int pid) {
    ...
    if (frame == -1) {
        printf("Error: No available frames for page fault handling\n");
        exit(1);
    }
